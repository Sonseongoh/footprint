# Design: Footprint, country-depth travel map

Generated by `/office-hours` on 2026-05-28
Branch: `master`
Repo: `footprint`
Status: APPROVED
Mode: Startup

## Problem Statement

특정 나라를 여러 번 여행하는 사용자는 "내가 이 나라를 얼마나 깊게 돌아다녔는지"를 도시 단위로 보기 어렵다. 기존 여행 기록 앱은 지도 가독성이 약하거나, 나라와 도시의 구분이 흐리거나, 도시별 방문 깊이를 다시 보고 싶게 만드는 힘이 약하다.

## Demand Evidence

아직 수요는 가설 단계다.

- 사용자는 여행 기록을 남기고 싶어 한다.
- 특히 한 나라 안에서 도시별로 얼마나 다녔는지 분리해서 보고 싶어 한다.
- 기존 대안으로 `Been`, `Polarsteps`를 알고 있지만, 아직 정확한 차별 지점은 검증되지 않았다.

## Status Quo

지금은 보통 기존 여행 앱, 사진첩, 메모, 기억에 의존해 여행 기록을 남긴다. 이 방식은 "나라 하나를 얼마나 깊게 탐험했는지"를 도시 단위 컬렉션으로 보여주기 어렵다.

## Target User & Narrowest Wedge

### Target User

특정 나라를 자주 여행하는 사람.

### Narrowest Wedge

사용자가 나라를 먼저 고르고, 실제 방문한 도시를 현장에서 위치 인증으로 체크한 뒤, 도시마다 짧은 메모와 사진 1장을 남기는 경험.

## Constraints

- 신뢰도가 편의보다 중요하다.
- 수동 추가는 거짓 기록을 만들 수 있으므로 기본 원칙에서 제외한다.
- 여행 중에도 쓸 수 있을 정도로 입력 마찰이 낮아야 한다.
- 지도 첫 화면이 충분히 직관적이고 예뻐야 다시 열어보게 된다.

## Premises

- 이 제품의 본질은 일반 여행일기가 아니라 "한 나라를 얼마나 깊게 탐험했는지"를 보여주는 것이다.
- 제품 구조는 `나라 먼저`가 맞다.
- 메인 감정은 `수집욕`이다.
- 도시 기록은 실제 현장 위치 인증 기반이어야 한다.
- 다만 여행 현실을 감안해, 사진 메타데이터나 위치 기록 같은 증거가 있으면 제한적 사후 복구는 허용한다.
- 같은 도시는 재방문이 누적되어야 한다.
- 공유는 도시 단건보다 나라별 채움 지도 결과물이 더 강하다.

## Approaches Considered

### Approach A: Country-depth collector

나라를 중심으로 컬렉션을 만들고, 도시를 실제 방문 기반으로 채워 넣는 지도형 기록 앱.

### Approach B: General travel log with recommendations

여행일기, 추천, 타임라인, 지도 기능을 넓게 섞는 범용 여행 앱.

## Recommended Approach

`Approach A`가 맞다.

이 제품의 가장 강한 루프는 명확하다.

`도시 기록` -> `나라 지도가 채워짐` -> `비어 있는 도시가 보임` -> `더 채우고 싶어짐` -> `추천을 보고 다음 여행으로 이어짐`

범용 여행 앱으로 가면 제품 설명이 약해지고, v1이 기능 모음집이 될 가능성이 크다.

## Product Shape

### Core Flow

1. 사용자가 관심 있는 나라를 고른다.
2. 현장에서 위치 인증으로 도시를 체크한다.
3. 체크 직후 한 줄 메모와 사진 1장을 남긴다.
4. 홈에서 그 나라의 방문 도시와 미방문 도시를 한눈에 본다.
5. 비어 있는 도시를 보며 다음 여행 욕구를 느낀다.

### Home

홈은 `지도 중심`이어야 한다. 첫 화면에서 사용자가 바로 느껴야 하는 감정은 "내가 이 나라를 이렇게 돌아다녔구나"다.

### Trust Model

- 기본: 현장 위치 인증
- 예외: 증거 기반 사후 복구
- 금지: 완전 자유 수동 추가

### Recording Model

- 도시 체크
- 한 줄 메모
- 사진 1장
- 재방문 누적

### Collection Model

- 사용자는 여러 나라를 각각 별도 컬렉션처럼 관리할 수 있다.
- 한 나라 안에서는 도시 단위 깊이가 핵심이다.

### Sharing

- 나라별 채움 지도를 이미지나 링크로 공유
- "내 일본 지도 봐" 같은 자랑 포인트가 전파 동력이 된다

## v1 Architecture Decisions (2026-06-02 office hours)

구현 계획(/plan-eng-review)으로 넘어가기 전에 아키텍처를 가르는 미결정을 확정했다.

- **v1 대상 나라**: 한국, 일본, 태국 3개로 한정. 도시 경계 데이터 범위를 좁혀 v1 구현을 가볍게 한다.
- **위치 인증 판정**: ~~도시 중심좌표 + 반경~~ → ~~행정구역(admin-1) 경계 polygon(2026-06-02)~~ → **변경(2026-07-14): 3국 모두 도시 경계 폴리곤 point-in-polygon으로 통일.** 수집 단위 = 채움 칸 = 게시판 키 = 도시 폴리곤 (KR 시 / JP 시구정촌, 도쿄 23구 병합 / TH 암프). 도시 폴리곤 밖은 어느 나라든 거절 — "가장 가까운 도시 스냅"(끄라비→푸껫 76km 거짓 기록의 원인)은 폐기. admin-1은 지도 백드롭과 상위 지역 표기로만 사용.
- **저장/동기화**: 처음부터 백엔드(Supabase 등). 링크 소셜 공유가 v1 요구사항이므로 로컬 전용은 불가.
- **공유 v1**: 링크 기반 소셜 공유 (공개 URL로 내 나라별 채움 지도를 공유). 이미지 캡처 공유도 부가로 가능.
- **사후 복구 최소 증거**: 사진 EXIF의 GPS 좌표 + 촬영 시각이 해당 도시 범위 안일 때만 사후 인정. 둘 중 하나라도 없으면 거부.
- **인증 마찰 주의**: ~~백엔드+소셜이 v1에 들어가면 계정/로그인 마찰이 "여행 중 입력 마찰 최소화" 원칙과 약하게 충돌한다. plan-eng-review에서 *익명 시작 후 나중에 계정 연결(deferred auth)* 패턴을 검토할 것.~~ → **변경(2026-07-02): deferred auth 폐기. 게스트 = 구경꾼(열람만), 체크인·좋아요·신고 등 모든 참여는 로그인 필수(RLS로 서버 강제).** 이유: 발자국은 계정에 속해야 하고(기기 아님), 익명 계정은 무한 생성이 가능해 좋아요 부풀리기·신고 3건 자동숨김 악용 벡터가 됨. 용어는 CONTEXT.md 참고.

## Visual Design (plan-design-review 2026-06-02)

방향 승인: **다크 나이트 + 골드 수집 포인트 + 게임풍 지구본**.

### 시각 시스템
- 팔레트: 배경 딥 나이트 네이비 `#0B1026`, 방문/채움 = 골드 `#F5C26B`(보물 수집 은유), 미방문 = 슬레이트 `#222B4D`/외곽 `#46517D`, 바다 = 틸→네이비 그라데이션. 액센트 1개(골드).
- 타이포: 디스플레이/숫자 = Space Grotesk, 본문 = Pretendard(한글+라틴). 본문 ≥ 16px, 대비 ≥ 4.5:1.
- 스페이싱: 4px 베이스. 미니멀 크롬, 카드 없이 지구본/지도를 포스터처럼.
- 모션: 지구본 느린 자동 회전 + 관성 드래그, 나라 탭 시 스프링 줌, 새 구역 채울 때 골드 "팝" 애니메이션.

### 화면 모델
- **진입 지구본**: 활성 3개국 골드 발광 핀, 상단 수집 진행 스탯, 하단 "지금 여기 체크인" 골드 필.
- **나라 도시 채움맵**: ~~추상 실루엣 + 점~~ → ~~행정구역(admin-1) choropleth~~ → **도시 경계 choropleth(2026-07-14)**. 방문 도시=골드 / 미방문=슬레이트 / 도시 아닌 땅=admin-1 백드롭(어두움, 채움 불가), 상단 "채운 도시 N/M", 도시 라벨은 줌에 따라 점진 노출.
- 채움 단위 = **도시 폴리곤** (KR 85 / JP 89 / TH 102개). 메모·사진은 도시 안의 깊이로 기록.
- **공유 웹**: 같은 도시 choropleth(채움+횟수만), 정확 위치·메모 비공개.

### 인터랙션 상태
- **첫 실행 빈 상태 = 온보딩 스포트라이트**: 지구본이 3개국을 스츠로 순회 + "여기부터 채워보세요" 가이드 + 0/N 진행 + 샘플 채움 미리보기. (설계 '30초 개념 이해'와 정합)
- 체크인: 로딩(GPS 잡는 중) / 지원 도시 아님(가까운 활성국 제안) / 권한 거부(설정 유도) / 성공(골드 팝 애니메이션).
- 지도: 동기화 중(은은한 스피너) / 오프라인(큐 대기 배지) / 빈 나라(온보딩 카피).
- 접근성: 터치 타겟 ≥ 44px, 본문 ≥ 16px, 대비 ≥ 4.5:1, 구역 탭은 라벨/상태 음성 안내.

### 와이어프레임
- `footprint-wireframe.png`(지구본 홈+점 버전), `footprint-map-v2.png`(행정구역 채움 버전). browse 캡처, 실제 미감 참고용.

## Open Questions

- `Been`, `Polarsteps`의 정확한 빈틈은 무엇인가? (직접 사용 과제 — 미완료)
- 도시별 반경을 어떻게 튜닝할 것인가? (대도시 vs 소도시 반경 차등)
- 사용자는 채운 비율, 방문 횟수, 지도 비주얼 중 무엇에 가장 반응하는가?
- ~~첫 번째로 가장 매력적으로 보여줄 나라는?~~ → 결정: 한국·일본·태국 3개
- ~~도시 경계 판정은 얼마나 정교해야?~~ → 결정: 중심좌표+반경
- ~~사후 복구에 필요한 최소 증거는?~~ → 결정: EXIF GPS+시각
- ~~개인 수집 vs 초기 소셜?~~ → 결정: 링크 소셜 공유를 v1에 포함

## Success Criteria

- 사용자가 30초 안에 앱의 개념을 이해한다.
- 여행 중 20초 안에 도시 인증과 기록 저장을 끝낼 수 있다.
- 사용자가 기록 입력뿐 아니라 지도 회상을 위해 앱을 다시 연다.
- 한 나라 안에서 방문 도시와 미방문 도시가 명확히 구분된다.
- 타깃 사용자 몇 명이 "이건 기존 앱보다 내가 한 나라를 깊게 본 느낌이 있다"고 말한다.

## Dependencies

- 한국·일본·태국 **도시 경계 GeoJSON**(cityareas — 채움+검증, mapshaper 단순화) + **행정구역(admin-1) GeoJSON**(백드롭·상위 지역명), 나라별 lazy-load. 출처: KR 통계청 / JP 국토수치정보 N03 / TH RTSD·UNOCHA COD
- point-in-polygon 모듈 (@turf/boolean-point-in-polygon 또는 robust-point-in-polygon, 직접 구현 금지)
- 위치 인증 시스템 (디바이스 GPS + 행정구역 polygon point-in-polygon, accuracy 게이트, "구역 없음" 처리)
- 사진 EXIF(GPS+촬영시각) 파싱 → 사후 복구 증거 판정
- 백엔드: Supabase 등 (Postgres + Auth + Storage) — 계정·동기화·공유 링크
- 사진 저장: 백엔드 스토리지
- 강한 지도 UI (Expo 56, 채움/미채움 도시 시각 구분)

## v56 API 검증 (2026-06-02, 추측 금지 — 문서 대조 완료)

구현 전 핵심 API를 Expo v56 / 각 라이브러리 문서로 확인. 옛 패턴 함정 표시(⚠️).

| 영역 | 확정 API (v56) | 함정 |
|---|---|---|
| 애니메이션 | Reanimated 4: `useSharedValue`/`useAnimatedStyle`/`useAnimatedProps`. SVG props는 `useAnimatedProps`+`createAnimatedPropAdapter`(`SVGAdapter`), 색은 `processColor` | ⚠️ `react-native-worklets` 별도 설치 필수(설치됨), **New Architecture(Fabric) 강제** → `app.json` newArchEnabled 확인 |
| 제스처 | `Gesture.Pan().onUpdate()` + `GestureDetector`, 루트에 `GestureHandlerRootView` 필수 | — |
| 로컬DB | expo-sqlite: `openDatabaseAsync`/`runAsync`/`getAllAsync`/`execAsync`/`prepareAsync` | — |
| 사진 리사이즈 | `ImageManipulator.manipulate().resize().renderAsync().saveAsync({format: SaveFormat.JPEG, compress})` | ⚠️ 옛 `manipulateAsync` **폐기됨** |
| 사진 선택/EXIF | expo-image-picker `launchImageLibraryAsync({exif:true})` → `exif`에 GPS | ⚠️ iOS 카메라 촬영분은 GPS 미포함(v1.1 복구는 라이브러리 사진만) |
| 위치 | expo-location `getCurrentPositionAsync({accuracy})`, `requestForegroundPermissionsAsync` | reverseGeocode는 iOS 네트워크 필요 → 도시명은 번들 포인트로 |
| 백엔드 | `createClient(url,key,{auth:{storage:LargeSecureStore, autoRefreshToken:true, persistSession:true, detectSessionInUrl:false}})` | SecureStore 2048byte 한계 → SecureStore(키)+AsyncStorage(세션) 조합 |
| 인증(deferred) | `signInAnonymously()` → `updateUser({email})` 또는 `linkIdentity({provider})` | — |
| 공유 웹 | `app.json` `expo.web.output:"static"`(또는 single), `npx expo export --platform web` → `eas deploy` | — |

### M1 추가 설치 필요 (현재 0개 설치됨)
`@supabase/supabase-js`, `@react-native-async-storage/async-storage`, `expo-secure-store`, `expo-location`, `expo-image-picker`, `expo-image-manipulator`, `expo-sqlite`, `react-native-svg`, `d3-geo`(+`@types/d3-geo`), `@turf/boolean-point-in-polygon`. (reanimated·gesture-handler·worklets는 이미 설치됨)

## The Assignment

`Been`, `Polarsteps`를 직접 써보고, 아래를 반드시 적는다.

1. "한 나라를 얼마나 깊게 탐험했는지"를 보여주는 데서 아쉬운 점 3개
2. 각 아쉬운 점의 화면 증거 또는 스크린샷
3. 그 빈틈이 진짜 문제인지, 아니면 내 취향인지에 대한 한 줄 판단

이걸 못 적으면 차별점 가설이 약한 것이다. 그때는 제품 정의를 다시 흔들어야 한다.

## What I noticed about how you think

- 사용자는 처음부터 일반 여행 앱이 아니라 `특정 나라를 자주 여행하는 사람` 쪽으로 문제를 좁혔다.
- `기록을 남기고 보면서 못가본 도시를 가고싶어지고 추천을 받고 싶어지기`라는 루프를 스스로 잡아냈다.
- 수동 추가가 정보 질을 떨어뜨린다고 보고 신뢰를 우선했다.
- 여러 기능을 퍼뜨리기보다 수집 구조를 계속 밀었다.
