...
- [ ] **차단(block) 기능** — Play UGC 정책은 신고+차단 둘 다 요구. 최소: 작성자 차단(로컬 or blocked_users 테이블) + getCityNotes 필터
- [ ] **웹 계정 삭제 경로** — Play 데이터 삭제 URL 요건. 최소: privacy 페이지에 "앱 없이 이메일로 삭제 요청" 명시 후 그 URL 제출
